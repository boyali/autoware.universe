...
template<typename T>
constexpr eigen_dynamic_type<T> block_diag(eigen_dynamic_type<T> const &m1, eigen_dynamic_type<T> const &m2)
{
...
/**
 * @brief Computes the cross product given two vectors in the form r0(x, y) = [x0, y0] and r1(x, y)
 * = [x1, y1] .
 *
 * */
template<typename T>
