...
void interp1d_map_linear(std::vector<double> const &sbase_coord_vect, std::vector<double> const &base_data_vect,
												 std::vector<double> const &snew_coords, std::vector<double> &new_data_to_be_interpolated)
{
	...
	new_data_to_be_interpolated.clear();
	new_data_to_be_interpolated.reserve(snew_coords.size()
	...
	for (double const &sk : snew_coords)
	{
		...
		auto const &&xk = base_data_vect[left] + ratio * (base_data_vect[right] - base_data_vect[left]);
...
void interp1d_linear(std::vector<double> const &sbase_coord_vector, std::vector<double> const &base_data_data_vector,
										 double const &snew_coord, double &new_data_to_be_interpolated)
{
	...
	// Get terminal data items.
	auto const &&xk = base_data_data_vector[left] + ratio * (base_data_data_vector[right] - base_data_data_vector[left]);
