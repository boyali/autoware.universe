																																...
																																VehicleDynamicsBase::state_vector_ad_t &f_xdot)
{
	...
	// auto yw = x(1);
	auto const &psi = x(2);  // heading angle.
	// auto s = x(3);   // distance travelled.
	auto const &ey = x(4);     // lateral error.
	auto const &epsi = x(5);   // heading error.
	...
	// Kinematic equations.
	f_xdot(0) = v * cos(beta + psi);  // !<@brief xw_dot
	f_xdot(1) = v * sin(beta + psi);  // !<@brief yw_dot
	// f_xdot(2) = v * sin(beta) / lr_;                         // !<@brief psi_dot
	f_xdot(2) = v * tan(delta) / wheel_base_;                   // !<@brief psi_dot
	f_xdot(3) = v * cos(beta + epsi) / (EPS + 1 - kappa * ey);  // !<@brief sdot
	f_xdot(4) = v * sin(beta + epsi);                           // !<@brief ey_dot
