...
observers::tf_t observers::get_nthOrderTF(
    const double &w_cut_off_hz, const int &n)
{
...
observers::tf_t observers::get_nthOrderTFwithDampedPoles(
    const double &w_cut_off_hz, const int &remaining_order,
...
void observers::LateralDisturbanceCompensator::simulateOneStep(const observers::state_vector_vehicle_t &current_measurements,
...
void observers::LateralDisturbanceCompensator::qfilterControlCommand(const double &current_control_cmd)
{
  // First give the output, then update the states.
  current_qfiltered_control_cmd_ = ss_qfilter_lat_.simulateOneStep(xu0_, current_control_cmd);
}
