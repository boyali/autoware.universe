...
: knots_ratio_(num_of_knots_ratio), compute_derivatives_{compute_derivatives}
{
  ...
  Eigen::MatrixXd tvec_base_normed(
    tvec_base.unaryExpr([&max_tvec_base](auto x) {return x / max_tvec_base;}));

  Eigen::MatrixXd tvec_new_normed(
    tvec_new.unaryExpr([&max_tvec_base](auto x) {return x / max_tvec_base;}));

  //  std::cout << "Normed tvec : \n";
  //  ns_eigen_utils::printEigenMat(tvec_base_normed);
  //
  //  std::cout << "Normed new tvec : \n";
  //  ns_eigen_utils::printEigenMat(tvec_new_normed);

  // Create knot points.
  nknots_ = static_cast<size_t>(static_cast<double>(tvec_base_normed.size()) * num_of_knots_ratio);
  knots_vec_ = ns_utils::linspace<double>(
    tvec_base_normed(0), tvec_base_normed(tvec_base_normed.rows() - 1), nknots_);

  //  ns_utils::print_container(knots_vec_);
  // Create the basis matrix from tvec_base to compute basis matrices for the base data.
  ...
  if (!compute_derivatives_) {
    ...
    // ns_eigen_utils::printEigenMat(basis_mat_dd);
    ...
    // ns_eigen_utils::printEigenMat(projection_mat_base_);
  ...
  const Eigen::VectorXd & tvec, Eigen::MatrixXd & basis_mat)
{
  ...
  basis_mat.col(2) = Eigen::VectorXd(tvec.unaryExpr([](auto const & x) {return x * x;}));

  basis_mat.col(3) = Eigen::VectorXd(tvec.unaryExpr([](auto const & x) {return x * x * x;}));
  ...
  Eigen::MatrixXd & regularization_mat_dd)
{
  ...
  basis_mat.col(2) =
    Eigen::VectorXd(tvec.unaryExpr([](auto const & x) {return x * x;}));    // [1, t, t**2]

  basis_mat.col(3) = Eigen::VectorXd(
    tvec.unaryExpr([](auto const & x) {return x * x * x;}));    // [1, t, t**2, t**3]
  ...
  //  ns_eigen_utils::printEigenMat(regularization_mat_dd);
  //  int a = 1;
}
  ...
  Eigen::MatrixXd & basis_ddmat)
{
  ...
  basis_dmat.col(3) = Eigen::VectorXd(tvec.unaryExpr([](auto const & x) {return 3 * x * x;}));
  ...
  const Eigen::MatrixXd & ybase, Eigen::MatrixXd & data_tobe_interpolated)
{
  ...
  for (size_t k = 0; k < numcols; k++) {
    ...
    ybase_normalized.col(k) =
      ybase.col(k).unaryViewExpr([&colmax](auto const & x) {return x / colmax;});
  }

  //  ns_eigen_utils::printEigenMat(ybase);
  //  ns_eigen_utils::printEigenMat(ybase_normalized);
  //  ns_eigen_utils::printEigenMat(projection_mat_w_new_base_);
  ...
  for (size_t k = 0; k < numcols; k++) {
    ...
    data_tobe_interpolated.col(k) = normalized_interpolated_data.col(k).unaryExpr(
      [colmax](auto const & x) {return x * colmax;});
  }

  //  ns_eigen_utils::printEigenMat(data_tobe_interpolated);
