...
void ns_splines::BSplineSmoother::createBasesMatrix(
  Eigen::MatrixXd const & tvec, Eigen::MatrixXd & basis_mat, Eigen::MatrixXd & basis_dmat,
  Eigen::MatrixXd & basis_ddmat)
{
...
std::vector<std::vector<double>> ns_splines::BSplineSmoother::basisRowsWithDerivatives(
  const double & ti)
{
  ...
  for (size_t k = 0; k < nknots_ - 2; k++) {
    ...
    std::vector<double> p_d1_d2 =
      fPlusCube(ti, ki);  // polynomial value with first and second derivatives.
...
void ns_splines::BSplineSmoother::solveByDemmlerReisch(
  Eigen::MatrixXd & basis_mat, Eigen::MatrixXd & basis_dmat, Eigen::MatrixXd & basis_ddmat)
{
  ...
  projection_mat_wb_ =
    basis_mat * projection_mat_base_;  // ynew = A{A_T A + lambda diag(s)}−1 A^T y.
...
void ns_splines::BSplineSmoother::InterpolateInCoordinates(
...
void ns_splines::BSplineSmoother::solveByQR(
  Eigen::MatrixXd & basis_mat, Eigen::MatrixXd & basis_dmat, Eigen::MatrixXd & basis_ddmat)
{
  //
...
void ns_splines::BSplineSmoother::getFirstDerivative(
  const Eigen::MatrixXd & ybase, Eigen::MatrixXd & ybase_dot)
{
  ybase_dot = projection_mat_dot_wb_ * ybase;
}

void ns_splines::BSplineSmoother::getSecondDerivative(
  const Eigen::MatrixXd & ybase, Eigen::MatrixXd & ybase_dot_dot)
{
