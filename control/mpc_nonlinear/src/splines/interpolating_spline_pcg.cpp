...
bool ns_splines::InterpolatingSplinePCG::compute_coefficients(std::vector<double> const &ybase)
{
	...
	if (interp_type_ == 1)
	{
		...
		std::transform(std::cbegin(ybase), std::cend(ybase), std::cbegin(ybase) + 1, std::back_inserter(b_coeff),
									 [](auto &v0, auto &v1)
									 { return v1 - v0; });
		...
		std::transform(std::cbegin(ybase), std::cend(ybase), std::cbegin(b_coeff), std::back_inserter(coefficients_),
									 [](auto &ai, auto &bi)
									 {
																												...
																												std::vector<double> const &c_coeffs)
{
	...
	// Set coeff d.
	std::transform(std::cbegin(c_coeffs), std::cend(c_coeffs), std::cbegin(c_coeffs) + 1,
