...
// Copy constructors.
ns_splines::InterpolatingSplinePCG::InterpolatingSplinePCG(const
                                                           ns_splines::InterpolatingSplinePCG &other) {
...
ns_splines::InterpolatingSplinePCG &
ns_splines::InterpolatingSplinePCG::InterpolatingSplinePCG::operator=(
        const InterpolatingSplinePCG &other) {
    if (this != &other) {
        interp_type_ = other.interp_type_;
        pcg_ = other.pcg_;
        coefficients_ = other.coefficients_;
...
// Move constructors.
ns_splines::InterpolatingSplinePCG::InterpolatingSplinePCG(
        ns_splines::InterpolatingSplinePCG &&other) noexcept {
    interp_type_ = other.interp_type_;
    pcg_ = other.pcg_;
...
void ns_splines::InterpolatingSplinePCG::set_bd_coeffs_(
    ...
    std::transform(
            ...
            std::back_inserter(d_coeffs), [](auto &c0, auto &c1) { return (c1 - c0) / 3; });
...
double ns_splines::InterpolatingSplinePCG::evaluatePolynomial(
        double const &ti, std::vector<double> const &coeffs) const {
...
bool ns_splines::PCG::solve(
    ...
    invDiag << Asparse.diagonal().unaryExpr([](auto const &x) { return 1 / x; });
