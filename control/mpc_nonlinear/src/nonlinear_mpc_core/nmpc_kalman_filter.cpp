...
void ns_filters::KalmanFilter::getStateEstimate(
...
void ns_filters::KalmanFilter::KalmanPredictionUpdateStep(
        const Model::input_vector_t &u0, Model::param_vector_t const &params) {
...
void ns_filters::KalmanUnscented::getStateEstimate(
...
void ns_filters::KalmanUnscented::KalmanUnscentedPredictionUpdateStep(
        const Model::input_vector_t &u0, Model::param_vector_t const &params) {
    ...
    auto const &&right_columns =
            Weight_common_ * sigmaPoints_fxfy_.rightCols(num_of_sigma_points_ - 1);
...
void ns_filters::KalmanUnscentedSQRT::getStateEstimate(
        Model::input_vector_t const &u0, Model::param_vector_t const &params,
        Model::state_vector_t const &x_measured, Model::state_vector_t &xest) {
    KalmanUnscentedPredictionUpdateStep(u0, params);
    KalmanUnscentedMeasurementUpdateStep(x_measured);
...
void ns_filters::KalmanUnscentedSQRT::choleskyOneRankUpdate(
...
void ns_filters::KalmanUnscentedSQRT::choleskyUpdateFromSigmaPoints(
    ...
    matrix_sqrt_tobe_updated = Model::state_matrix_t(
            Eigen::MatrixXd(qr.matrixQR().triangularView<Eigen::Upper>()).topRows(Model::state_dim));
...
void ns_filters::KalmanUnscentedSQRT::KalmanUnscentedPredictionUpdateStep(
        const Model::input_vector_t &u0, Model::param_vector_t const &params) {
    generateSigmaPoints(sigmaPointsMat_x_);  // <-@brief generate sigma points of UKF
    propagateSigmaPoints_fx(u0, params);     // <-@brief propagate sigma points X =\int f(x)
...
void ns_filters::KalmanUnscentedSQRT::KalmanUnscentedMeasurementUpdateStep(
        const Model::state_vector_t &x_measured) {
    generateSigmaPoints(sigmaPointsMat_y_);
