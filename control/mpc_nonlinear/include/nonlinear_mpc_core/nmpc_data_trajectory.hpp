...
public:
  ...
  std::vector<double> acc;        //!< @brief implied acceleration [m/s/s]
   ...
   */
  void emplace_back(autoware_planning_msgs::msg::Trajectory const & msg);
...
// u_model_solution_ = [acc, steering_rate]
template<class Model>
void TrajectoryData<Model>::setFeedbackControls(
  typename Model::input_vector_t const & u_feedback_model_msg)
{
  u_model_solution_ = u_feedback_model_msg;
}

template<class Model>
void TrajectoryData<Model>::getFeedbackControls(
  typename Model::input_vector_t & u_feedback_solution_msg) const
{
