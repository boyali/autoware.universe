 ...
 * */
    ...
    public:
        ...
        void getStateEstimate(
                Model::input_vector_t const &u0, Model::param_vector_t const &params,
    ...
    private:
        ...
        // Class methods
        void KalmanPredictionUpdateStep(
    ...
    public:
        ...
        explicit KalmanUnscented(Model::model_ptr_t const &model, double const dt)
                : model_ptr_(model), dt_{dt} {
            V_.setIdentity();
            W_.setIdentity();
            P_.setZero();
    ...
    private:
        ...
        Model::state_matrix_t V_{
                Model::state_matrix_t::Identity()};  // !<-@brief process uncertainty covariance matrix.
        Model::state_matrix_t W_{
                Model::state_matrix_t::Identity()};  // !<-@brief measurement uncertainty covariance matrix.
        Model::state_matrix_t P_{Model::state_matrix_t::Zero()};  // !<-@brief initial and updated covariance.
        Model::state_matrix_t Psqrt_{
                Model::state_matrix_t::Zero()};  // !<-@brief square root of the covariance matrix P_.
        ...
        // Placeholders
        sigma_point_mat_t sigmaPointsMat_x_{
                sigma_point_mat_t::Zero()};  // !<-@brief sigma points generated from the states.
        sigma_point_mat_t sigmaPointsMat_y_{
                sigma_point_mat_t::Zero()};  // !<-@brief sigma points generated from the measurements.
    ...
    private:
        ...
        Model::state_matrix_t Vsqrt_{
                Model::state_matrix_t::Identity()};  // !<-@brief process uncertainty sqrt covariance matrix.
        Model::state_matrix_t Wsqrt_{
                Model::state_matrix_t::Identity()};  // !<-@brief measurement uncertainty sqrt covariance matrix.

        // Frequently re-assigned.
        Model::state_matrix_t Sx_sqrt_{
                Model::state_matrix_t::Zero()};  // !<-@brief square root of the covariance matrix P_.
        Model::state_matrix_t Sy_sqrt_{Model::state_matrix_t::Zero()}; // !<-@brief measurement covariance square root
        ...
        **/
        ...
        void choleskyOneRankUpdate(
                Model::state_matrix_t &R, Eigen::MatrixBase<Derived> const &U, double const &weight);

        void choleskyUpdateFromSigmaPoints(
                Model::state_matrix_t &matrix_sqrt_tobe_updated, sigma_point_mat_t const &sigma_points,
                sigma_concat_mat_t &sigma_concatenated, double const &weight);
    };
