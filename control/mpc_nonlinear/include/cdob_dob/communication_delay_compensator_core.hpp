...
#include "vehicle_kinematic_error_model.hpp"
#include "utils.h"
...
class LateralDisturbanceCompensator
{
 ...
 private:
  ...
  // placeholders
  measurement_matrix_observer_t Lobs_;    //@brief state observer gain matrix.
  state_vector_observer_t theta_params_;  //@brief nonlinear terms in A of SS models of vehicle.
...
tf_t get_nthOrderTFwithDampedPoles(
    double const &w_cut_off_hz, int const &remaining_order, double const &damping_val);
