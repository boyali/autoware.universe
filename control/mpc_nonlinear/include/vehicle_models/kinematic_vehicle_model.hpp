...
namespace ns_models {
    struct ParamsVehicle {
        ParamsVehicle()
                : wheel_base{2.7}, lr{1.4}, steering_tau{0.27}, speed_tau{0.1}, use_delay_model{true} {
        }
        ...
        double wheel_base;
        double lr;
        double steering_tau;   // !<@brief First order steering system model time constant.
        double speed_tau;      // !<@brief Speed time constant.
        bool use_delay_model;  // !<@brief use time constant in the steering and long dynamics.
    };
    ...
    public:
         ...
         * */
        [[nodiscard]] double getSteeringDynamics_deltadot(
                double const &current_steering, double const &current_steering_input) const;

        /**
         * @brief returns rate of change of speed Vx state.
         * Get speed dynamics v_dot.
         * */
        [[nodiscard]] double getLongSpeedDynamics_vdot(
                double const &current_long_speed, double const &current_speed_input) const;
