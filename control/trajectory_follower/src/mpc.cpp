			...
			namespace trajectory_follower
			{
						...
						autoware_auto_system_msgs::msg::Float32MultiArrayDiagnostic& diagnostic)
				{
					...
					trajectory_follower::MPCTrajectory reference_trajectory =
							                                   applyVelocityDynamicsFilter(m_ref_traj, current_pose,
									                                   current_velocity);
					...
					Eigen::VectorXd                    Xex = mpc_matrix.Aex * x0 + mpc_matrix.Bex * Uex +
					                                         mpc_matrix.Wex;
				...
				{
					const auto                                               time_delayed = m_clock->now() +
						...
						const float64_t v0) const
				{
					...
					output.push_back(
							output.x.back(), output.y.back(), output.z.back(), output.yaw.back(), v_end,
							output.k.back(),
 ...
 */
				...
				void MPC::addSteerWeightR(Eigen::MatrixXd* R_ptr) const
				{
					...
					/* add steering rate : weight for (u(i) - u(i-1) / dt )^2 */
					{
						...
						const Eigen::Matrix2d D            = steer_rate_r *
					...
					/* add steering acceleration : weight for { (u(i+1) - 2*u(i) + u(i-1)) / dt^2 }^2 */
					{
						...
						const Eigen::Matrix3d D               =
								                      steer_acc_r *
								                      (Eigen::Matrix3d()
										                      << 1.0, -2.0, 1.0, -2.0, 4.0, -2.0, 1.0, -2.0, 1.0).finished();
				...
				bool8_t MPC::isValid(const MPCMatrix& m) const
				{
					if (
							...
							m.Wex.array().isNaN().any() || m.Qex.array().isNaN().any() ||
					...
					if (
							...
							m.Wex.array().isInf().any() || m.Qex.array().isInf().any() ||
							m.R1ex.array().isInf().any() ||
