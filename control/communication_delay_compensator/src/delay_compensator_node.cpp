...
#include "delay_compensator_node.hpp"
...
DelayCompensationNode::DelayCompensationNode(const rclcpp::NodeOptions & node_options)
...
void DelayCompensationNode::initTimer(float64_t period_s)
{
  const auto period_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
    std::chrono::duration<float64_t>(period_s));

  m_timer_ = rclcpp::create_timer(
    this, get_clock(), period_ns, std::bind(&DelayCompensationNode::onTimer, this));
}
void DelayCompensationNode::onTimer()
{
...
RCLCPP_COMPONENTS_REGISTER_NODE(
  autoware::motion::control::observer::delay_compensation_nodes::DelayCompensationNode)