  ...
  [[maybe_unused]] tier4_planning_msgs::msg::StopReason * stop_reason)
{
  ...
  utils::buildDetectionAreaPolygon(
    debug_data_.detection_areas, interp_path, offset_from_start_to_ego, param_);
  ...
  utils::createPossibleCollisionsInDetectionArea(
    debug_data_.detection_areas, possible_collisions, grid_map, interp_path, offset_from_start_to_ego,
    param_, debug_data_.occlusion_points);
  if (debug_data_.detection_areas.empty()) return true;
