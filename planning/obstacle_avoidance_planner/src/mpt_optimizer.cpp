  ...
  std::shared_ptr<DebugData> debug_data_ptr)
{
  ...
  std::vector<ReferencePoint> full_ref_points =
    getReferencePoints(smoothed_points, prev_trajs, enable_avoidance, maps, debug_data_ptr);
  ...
  const auto mpt_matrix = generateMPTMatrix(non_fixed_ref_points, debug_data_ptr);

  // calculate Q and R matrices
  const auto val_matrix = generateValueMatrix(non_fixed_ref_points, path_points, debug_data_ptr);

  const auto optimized_control_variables = executeOptimization(
    prev_trajs, enable_avoidance, mpt_matrix, val_matrix, non_fixed_ref_points, debug_data_ptr);
  ...
  const std::unique_ptr<Trajectories> & prev_trajs) const
{
  ...
  for (size_t i = begin_prev_ref_idx; i <= static_cast<size_t>(end_prev_ref_idx); ++i) {
    ...
    if (std::abs(lat_distance) < 0.05) {
      ...
      /*
      RCLCPP_ERROR_STREAM(
        rclcpp::get_logger("fix_kinematic_state"), i << " " <<
      fixed_ref_point.fix_kinematic_state.get()(0) << " "
                                           << fixed_ref_point.fix_kinematic_state.get()(1));
      */
  ...
  std::shared_ptr<DebugData> debug_data_ptr) const
{
  ...
  for (size_t i = 0; i < N_ref; ++i) {
    ...
    // TODO(murooka) add decreasing weights in a narrow and sharp curve
    /*
      ...
      for (size_t s_idx = 0; s_idx < N_soft; ++s_idx) {
        ...
        if (s_idx == 1) {
          ...
          lb_blk.segment(0, N_ref) -=
            Eigen::MatrixXd::Constant(N_ref, 1, mpt_param_.soft_second_clearance_from_road);
  ...
  const MPTMatrix & mpt_mat, std::shared_ptr<DebugData> debug_data_ptr)
{
  ...
  /*
  // TODO(murooka) should recalculate orientation?
  for (size_t i = 0; i < lat_error_vec.size(); ++i) {
    auto & ref_point = (i < fixed_ref_points.size())
                         ? fixed_ref_points.at(i)
                         : non_fixed_ref_points.at(i - fixed_ref_points.size());
...
void MPTOptimizer::calcExtraPoints(std::vector<ReferencePoint> & ref_points) const
{
  for (size_t i = 0; i < ref_points.size(); ++i) {
    ...
    const double front_wheel_s =
      ref_points.at(i).s + vehicle_param_.wheelbase;  // TODO(murooka) use offset?
