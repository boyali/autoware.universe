...
bool isPathShapeChanged(
  ...
  const std::unique_ptr<std::vector<autoware_auto_planning_msgs::msg::PathPoint>> &
    prev_path_points,
  const double distance_for_path_shape_change_detection, const double delta_yaw_threshold)
{
  if (!prev_path_points) {
    return false;
  }
    ...
    prev_path_points)
{
  ...
  // path may be cropped and does not contain the goal.
  const double path_length = tier4_autoware_utils::calcArcLength(path_points);
  const double prev_path_length = tier4_autoware_utils::calcArcLength(*prev_path_points);
  if (path_length > 50 && prev_path_length > 50) {
    return false;
  }
  ...
  eb_solved_count_(0)
{
  ...
  {  // trajectory parameter
    ...
    traj_param_.center_line_width = declare_parameter<double>("object.center_line_width");
  ...
  {  // mpt param
    ...
    // option
    mpt_param_.l_inf_norm = declare_parameter<bool>("mpt.option.l_inf_norm");
    ...
    mpt_param_.soft_constraint = declare_parameter<bool>("mpt.option.soft_constraint");
    mpt_param_.hard_constraint = declare_parameter<bool>("mpt.option.hard_constraint");
  ...
  const std::vector<rclcpp::Parameter> & parameters)
{
  ...
  {  // trajectory parameter
    ...
    updateParam<double>(
      ...
      traj_param_.max_avoiding_objects_velocity_ms);
    updateParam<double>(parameters, "object.center_line_width", traj_param_.center_line_width);
    updateParam<bool>(
  ...
  {  // mpt param
    // option
    updateParam<bool>(parameters, "mpt.option.l_inf_norm", mpt_param_.l_inf_norm);
    updateParam<bool>(
      parameters, "mpt.option.two_step_soft_constraint", mpt_param_.two_step_soft_constraint);
    updateParam<bool>(parameters, "mpt.option.plan_from_ego", mpt_param_.plan_from_ego);
    updateParam<bool>(parameters, "mpt.option.soft_constraint", mpt_param_.soft_constraint);
    updateParam<bool>(parameters, "mpt.option.hard_constraint", mpt_param_.hard_constraint);
    updateParam<bool>(
  ...
  const autoware_auto_planning_msgs::msg::Path & path)
{
  ...
  // retun prev trajctory if replan is not required
  ...
  const std::vector<autoware_auto_planning_msgs::msg::PathPoint> & path_points)
{
  ...
  // For when path from behavior changes, just replan and not intialize optimization
  ...
  const std::vector<autoware_auto_planning_msgs::msg::TrajectoryPoint> & optimized_points)
{
  ...
  auto extended_traj_points =
    [&]() -> std::vector<autoware_auto_planning_msgs::msg::TrajectoryPoint> {
    constexpr double non_fixed_traj_length = 5.0;  // TODO(murooka)
  ...
  const std::vector<autoware_auto_planning_msgs::msg::TrajectoryPoint> & traj_points) const
{
  ...
  const size_t zero_vel_fine_traj_idx = [&]() {
    ...
    const size_t zero_vel_traj_idx =
      searchExtendedZeroVelocityIndex(fine_traj_points, traj_points);  // for outside drviable area
